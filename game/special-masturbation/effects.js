...
function masturbationEffects() {
	...
	if (otherVariables.additionalEffect.hands === "ballplayeffects" && V.worn.genitals.name !== "chastity parasite") {
		if (V.arousal >= (C.arousal.max * 0.8) || (V.earSlime.focus === "impregnation" && V.earSlime.growth >= 100)) {
			if (genitalsExposed()) {
		...
		} else if (V.arousal >= (C.arousal.max * 0.6)) {
			if (genitalsExposed()) {
		...
		} else if (V.arousal >= (C.arousal.max * 0.4)) {
	...
	switch (V[armAction]) {
		...
		case "mchest":
			...
			if (V.worn.over_upper.exposed >= 2 && V.worn.upper.exposed >= 2 && V.worn.under_upper.exposed >= 1) {
				...
				if (V.lactating && V.settings.breastFeedingEnabled === true && V.bugsinside) {
					if (V.arousal >= (C.arousal.max * 0.8)) {
						sWikifier(
		...
		case "mchastityparasiterub":
			...
			if (earSlimeDefy()) {
				...
				altText.eagerly = V.arousal >= (C.arousal.max * 0.2) ? "eagerly" : "slowly";
				wikifier("arousal", 500, "masturbationPenis");
				V.earSlime.vibration += handsOn * 4;
				if (V.arousal >= (C.arousal.max * 0.6)) {
					wikifier("arousal", 500, "masturbationPenis");
					sWikifier(
		...
		case "mballssqueeze":
			...
			if (earSlimeDefy()) {
				...
				wikifier("arousal", 200 * handsOn, "masturbationPenis");
				altText.gently = V.arousal >= (C.arousal.max * 0.8) ? "urgently" : V.arousal >= (C.arousal.max * 0.6) ? "" : "gently";
				if (handsOn === 2) {
					switch (V.ballssize) {
						case 3:
						case 4:
	...
	switch (V.mouthaction) {
		...
		case "mpenissuck":
			...
			if (earSlimeDefy()) {
				...
				altText.eagerly = V.arousal >= (C.arousal.max * 0.4) ? "eagerly" : "slowly";
				if (V.arousal >= (C.arousal.max * 0.8)) {
