...
Macro.add("relativeInput", {
	...
	handler() {
		...
		const minutes = $(`<option value="minutes">Minutes</option>`);
		const hours = $(`<option value="hours">Hours</option>`);
		const days = $(`<option value="days">Days</option>`);
		granularity.append(minutes).append(hours).append(days);
