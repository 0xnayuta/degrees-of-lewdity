Weather.Renderer.Effects.add({
	...
	defaultParameters: {
		...
		maxRecursion: 1, // currently unused
		...
		// speed
		boltStepsPerFrame: 5, // how many pixels to draw per animation frame
		boltSpeedFactor: 5, // multiplier on stepsPerFrame

		// hold bright on completion - probably replace?
	...
	draw() {
		...
		// spawn new bolt
		this.boltTimer += dt;
		if (this.boltTimer >= this.lightningInterval) {
			this.boltTimer -= this.lightningInterval;
			...
			const endY = this.boltTargetY != null ? this.boltTargetY : viewHeight - 20; // default above bottom

			const x = endX - x0;
			const y = endY - y0;
			const boltLength = Math.sqrt(x * x + y * y);
