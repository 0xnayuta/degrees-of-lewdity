 ...
 * @param {string} donorSpecies the donor's species, what the pregnancy grows as
...
function createPregnancy(carrier, carrierSpecies, donor, donorSpecies, possibleDonors, conceivedDate, orifice, conceivedLocation) {
	...
	if (carrier === "pc") V.sexStats.vagina.menstruation.currentState = "pregnant";
	return pregnancyId;
}
 ...
 * @param {string} species the child's species, e.g. "human", "wolf", "hawk"
 ...
 *
 * @param {string} species the child base species, e.g. "human", "wolf", "hawk"
...
function checkLabour() {
	if (V.combat === 1 || V.possessed) return;
...
function clearPregnancyRecord(pregnancyId) {
	V.pregnancies[pregnancyId].carrier = "cleared";
}
...
/**
 * A child's siblings (same pregnancy), excluding the child itself.
 *
 * @param {Child} child the child whose littermates to look up
 */
function getSiblingsOf(child) {
	return getChildrenOf(child.pregnancyId).filter(other => other.childId !== child.childId);
}
window.getSiblingsOf = getSiblingsOf;

/**
 * A child's identical twins (littermates in the same identical set), empty if none.
 *
 * @param {Child} child the child whose identical-twin set to look up
 */
function getIdenticalTwinsOf(child) {
	if (child.identical === null) return [];
	return getSiblingsOf(child).filter(other => other.identical === child.identical);
}
window.getIdenticalTwinsOf = getIdenticalTwinsOf;
...
/**
 * Whether someone knows who's carrying a pregnancy.
 *
 * @param {number} pregnancyId the pregnancy's index in V.pregnancies
 * @param {string} who "pc", or an NPC's name
 */
...
// Whether each person has discussed this pregnancy, keyed by name ("pc" or an NPC's). Scenes only
// ever ask yes/no, so these are presence flags. (!! folds any leftover count from an old save to a boolean.)
...
function hasTalkedAbout(pregnancyId, who) {
	return !!V.pregnancies[pregnancyId].talkedAbout[who];
}
