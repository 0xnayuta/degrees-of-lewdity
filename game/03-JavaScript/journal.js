...
class CustomReminder {
	...
	constructor(
		...
		if (
			...
			timeStamp < TimeConstants.MIN_DATE.timeStamp ||
			timeStamp > TimeConstants.MAX_DATE.timeStamp
		)
		...
		if (this.timeStamp !== 0 && this.repeats !== 0 && (!Number.isFinite(cooldown) || typeof cooldown !== "number"))
			throw new Error(
				`invalid cooldown ${cooldown} passed to CustomReminder constructor with nonzero timeStamp ${this.timeStamp} and nonzero repeats ${this.repeats}`
			);
		/**
		 * @type {DateTime.timeStamp}
		 * @description
		 ...
		 */
		this.intrusive = !!intrusive; // coerce into boolean
		 ...
		 * must be false if intrusive is false
		 */
		this.persistent = !!persistent; // more coercion
		...
		if (typeof entry !== "string") throw new Error(`non-string entry ${entry} passed to CustomReminder constructor`);
		if (entry.includes("nowiki")) {
			// this should be good enough to avoid unexpected wikification effects, but it'll need bug-testing
			entry = "nice try ;)";
		}
		/**
		 * @type {string}
	 ...
	 *
	 * @returns {string} the text of the reminder that we want to display in
	 * 					 the journal, wrapped up cozy and safe
	 */
	getText() {
	...
	update() {
		...
		if (V.customReminders?.reminders === undefined)
			throw new Error(
				`V.customReminders exists but has no attribute reminders, yet some CustomReminder with entry ${this.entry} exists and called its update()`
			);
		if (V.customReminders?.notifications === undefined) throw new Error(`V.customReminders exists but has no attribute notifications`);
		...
		if (this.hidden || this.expired) return;
		...
		if (Time.date.timeStamp >= this.timeStamp + this.duration) {
			const elapsedTime = Time.date.timeStamp - this.timeStamp; // current time - fire time
		...
		if (Time.date.timeStamp >= this.timeStamp + this.duration) {
			...
			if (this.repeats !== -1) this.repeats--;
...
Macro.add("journalCustomEntriesRepeatsInput", {
	tags: null,
	handler() {
		if (this.args.length !== 3) return this.error("<<journalCustomEntriesRepeatsInput>> had bad arguments. bad args: " + JSON.stringify(this.args));
		...
		const label = $("<label></label>").append(checkboxInput).append(`<span> Repeat forever</span>`);
